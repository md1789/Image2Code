...
export default function Register() {
  const { registerWithEmail, signInWithGoogle, loading, user, error } = useAuth();
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!email || !password) {
      setFormError("Please provide an email address and password.");
      return;
    }
    ...
    try {
      ...
      if (registerError instanceof Error) {
        ...
        setFormError("We couldn't create your account right now. Please try again later.");
      }
    } finally {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (registerError instanceof Error) {
        ...
        setFormError("We couldn't create your account with Google. Please try again.");
      }
    } finally {
      setIsSubmitting(false);
    }
      ...
      <section className="w-full max-w-md rounded-2xl border border-slate-800/60 bg-slate-900/70 p-8 shadow-xl shadow-black/40">
        <header className="mb-6 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Start generating UI code with <span className="text-[#6FA3FF]">Image2Code</span>.
          </p>
        </header>
        ...
        <p className="mt-6 text-center text-sm text-slate-400">
          ...
          <Link className="text-[#6FA3FF] hover:underline" to="/login">
            Sign in
          </Link>
        </p>
      </section>
